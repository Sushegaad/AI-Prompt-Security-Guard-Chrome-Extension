...
**Shield Mode (optional, off by default).** For users who want drafts protected even before send, Shield Mode lets them type inside an extension-origin iframe overlaid on the provider's composer. Same-origin policy prevents the provider's page scripts from reading it; only user-approved (optionally redacted) text is relayed through the service worker to the content script and injected into the real composer — never via `window.postMessage`, and every relay message carries a per-session nonce. The frame's only other message is a layout report (`SHIELD_RESIZE`): a sanitized pixel number, never content. It adds no permission (the iframe rides on the existing `web_accessible_resources`), makes no network calls, and stores only a per-site on/off flag.
