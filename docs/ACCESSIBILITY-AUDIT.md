...
- **Focus management.** The secure composer opens only from an explicit activation of the always-visible "Shield" chip (a native `<button>` with a descriptive `aria-label`, pinned to the composer corner) — a user-initiated context change (3.2.1) with no automatic focus stealing. Opening moves focus into the iframe input; on close, focus is restored to the real composer except after an approved send, where the site's own post-send focus behavior is respected.
